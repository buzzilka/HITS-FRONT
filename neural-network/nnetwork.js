...
// Функция центрирования изображения в canvas
function imageCentering(imgData, image) {
...
function neuralNetwork() {
    let image = new Image();
    image.src = canvas.toDataURL(); // Преобразование canvas в изображение для обработки

    image.onload = () => {
        var imgData = context.getImageData(0, 0, imgParametrs, imgParametrs).data;
        let scaledImage = imageCentering(imgData, image); // Центрирование и масштабирование изображения

        let userImage = new Array(imgParametrs ** 2);
        for (let i = 3; i < scaledImage.length; i += 4) {
            userImage[Math.floor(i / 4)] = (scaledImage[i] / 255); // Преобразование цветов изображения для передачи в нейронную сеть
        }

        const output = nnetwork.feedForward(userImage); // Получение выходных данных нейронной сети
...
// Функция обрезки изображения по оси X
function cropByX(fromLeft, imgData) {
    var offset = fromLeft ? 1 : -1; 

    var result = null;
    if (fromLeft) {
        // Поиск границы изображения слева
        ...
        // Поиск границы изображения справа
...
// Функция обрезки изображения по оси Y
function cropByY(fromTop, imgData) {
    var offset = fromTop ? 1 : -1; 

    var result = null;
    if (fromTop) {
        // Поиск границы изображения сверху
        ...
        // Поиск границы изображения снизу
